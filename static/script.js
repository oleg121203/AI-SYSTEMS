...
function connectWebSocket() {
    ...
    if (logContent) logContent.innerHTML += `<p><em>Attempting to connect to WebSocket: ${wsUrl}</em></p>`;

    ws = new WebSocket(wsUrl);

    ws.onopen = function(event) {
        console.log("WebSocket connection opened");
        if (logContent) logContent.innerHTML += '<p><em>WebSocket connection established</em></p>';
        showNotification("Connected to server", "success");
        reconnectAttempts = 0; // Reset attempts on successful connection
        // Request initial full status upon connection
        // ws.send(JSON.stringify({ action: 'get_status' })); // If backend supports this
    };
        ...
        try {
            ...
            switch (data.type) {
                ...
                default:
                    ...
                    if (data.ai_status || data.queues || data.subtasks || data.structure) {
            ...
            console.error("Error parsing WebSocket message or updating UI:", e, "Raw data:", event.data);
            if (logContent) logContent.innerHTML += `<p><em><strong style="color:red;">Error parsing WebSocket message:</strong> ${e}</em></p>`;
        }
    };

    ws.onerror = function(event) {
        console.error("WebSocket error observed:", event);
        if (logContent) logContent.innerHTML += '<p><em><strong style="color:red;">WebSocket error.</strong></em></p>';
        showNotification("WebSocket error", "error");
    };

    ws.onclose = function(event) {
        console.log("WebSocket connection closed. Code:", event.code, "Reason:", event.reason);
        if (logContent) logContent.innerHTML += `<p><em>WebSocket connection closed. Attempting to reconnect... (${reconnectAttempts + 1}/${maxReconnectAttempts})</em></p>`;
        ...
        if (reconnectAttempts < maxReconnectAttempts) {
            ...
            if (logContent) logContent.innerHTML += `<p><em><strong style="color:red;">Failed to reconnect after ${maxReconnectAttempts} attempts.</strong> Please refresh the page.</em></p>`;
...
function updateFullUI(data) {
    ...
    if (data.queues) { // Assuming data.queues is { executor: [...], tester: [...], ... }
        updateQueues(data.queues);
    }
...
function updateStatsFromSubtasks(subtasks) {
    ...
    const completed = Object.values(subtasks).filter(status =>
        status === 'accepted' || status === 'completed' || status === 'code_received' // Consider these as completed for stats
    ).length;
    ...
    if (statElements.efficiency) statElements.efficiency.textContent = `${efficiency}%`;
}

// Fallback if 'subtasks' field isn't in the data
function updateStatsLegacy(data) {
     if (statElements.total && data.total_tasks !== undefined) statElements.total.textContent = data.total_tasks;
     if (statElements.completed && data.processed !== undefined) statElements.completed.textContent = data.processed;
     if (statElements.efficiency && data.efficiency !== undefined) statElements.efficiency.textContent = data.efficiency;
}
...
function getStatusIcon(status) {
    switch (status) {
        case 'pending': return '⏳';
        case 'processing': return '<i class="fas fa-spinner fa-spin"></i>'; // Font Awesome spinner
        case 'accepted':
        case 'completed':
...
function updateCharts(data) {
        ...
        if (ctx) {
            taskChart = new Chart(ctx, {
                ...
                options: {
                    ...
                    scales: { y: { beginAtZero: true, ticks: { color: getChartFontColor() } } , x: { ticks: { color: getChartFontColor() } }},
    ...
    if (!progressChart) {
         ...
         if (ctx) {
            progressChart = new Chart(ctx, {
                ...
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    scales: { y: { beginAtZero: true, max: 100, ticks: { color: getChartFontColor() } }, x: { ticks: { color: getChartFontColor() } } },
                    plugins: { legend: { labels: { color: getChartFontColor() } } }
                }
            });
     ...
     if (progressChart && data.progress && data.progress.stages && data.progress.values) {
...
function updateFileStructure(structureData) {
    ...
    if (!structureData || Object.keys(structureData).length === 0) {
        fileStructureDiv.innerHTML = '<p><em>Project structure is empty or unavailable.</em></p>';
        return;
    }
    ...
    function renderNode(node, parentUl, currentPath = '') {
        ...
        for (const [key, value] of entries) {
            ...
            const isDirectory = typeof value === 'object' && value !== null && Object.keys(value).length >= 0; // Treat empty objects as folders too
            ...
            if (isDirectory) {
                ...
                li.innerHTML = `<span class="file" data-path="${itemPath}"><i class="fas ${getFileIcon(key)}"></i> ${key}</span>`;
...
function getFileIcon(filename) {
    ...
    switch (ext) {
        case 'py': return 'fa-python fab'; // Use Font Awesome Brands for Python
        case 'js': return 'fa-js fab';
        case 'html': return 'fa-html5 fab';
        case 'css': return 'fa-css3-alt fab';
        case 'json': return 'fa-file-code'; // Generic code icon
        case 'md': return 'fa-markdown fab';
        case 'txt': return 'fa-file-alt';
        case 'png': case 'jpg': case 'jpeg': case 'gif': case 'svg': return 'fa-file-image';
        case 'dockerfile': return 'fa-docker fab';
        case 'gitignore': return 'fa-git-alt fab';
        default: return 'fa-file'; // Default file icon
    }
}
...
async function loadFileContent(path) {
    ...
    try {
        ...
        const response = await fetch(`/file_content?path=${encodeURIComponent(safePath)}`);
        ...
        if (response.ok) {
            ...
            console.log(`File content loaded successfully for ${path}, language set to ${language}`);
            showNotification(`Loaded ${path}`, "info");
        } else {
            const errorText = await response.text();
            editor.setValue(`// Failed to load file: ${path}\n// Status: ${response.status}\n// ${errorText}`);
            showNotification(`Failed to load file: ${path} (${response.status})`, 'error');
            console.error("Failed to load file content, status:", response.status, "Response:", errorText);
        }
    } catch (error) {
...
function getEditorTheme(appTheme) {
     ...
     return (appTheme === 'dark' || appTheme === 'winter' || appTheme === 'autumn') ? 'vs-dark' : 'vs-light';
}
...
// --- Notifications ---
function showNotification(message, type = 'info') { // success, error, warning, info
...
async function resetSystem() {
    if (confirm("Are you sure you want to reset the system? This will clear queues, logs, and restart AI processes.")) {
        try {
