...
from utils import SystemMonitor, TestValidator, log_message
...
class ProjectStructure:
    ...
    async def _scan_directory(
        self, dir_path: Path, rel_path: str = ""
    ) -> Dict[str, Any]:
        """Recursively scan directory and build structure"""
        if not dir_path.exists() or not dir_path.is_dir():
            return {}

        result = {"type": "directory", "name": dir_path.name, "children": []}
    ...
    async def update_from_spec(self, spec: Dict[str, Any]):
        ...
        self.history.append(
            {"timestamp": time.time(), "spec": spec, "action": "update_from_spec"}
        )
        ...
        for item_name, item_data in top_structure.items():
            if (
                ...
                and "type" in item_data
                and item_data["type"] == "directory"
            ):
    ...
    async def _process_directory_children(
        self, parent_path: Path, parent_rel_path: str, children: List[Dict[str, Any]]
    ):
        ...
        for child in children:
            ...
            if child.get("type") == "directory":
                ...
                if "children" in child and isinstance(child["children"], list):
                    await self._process_directory_children(
                        child_path, child_rel_path, child["children"]
                    )
    ...
    async def _create_file(
        self, file_path: Path, rel_path: str, content: str, description: str = ""
    ):
    ...
    async def analyze_dependencies(self):
        ...
        log_message(
            f"Analyzed dependencies, found {sum(len(deps) for deps in self.dependencies.values())} relationships"
        )
                ...
                for imp in imports:
                    ...
                    if imp.startswith("."):
                        ...
                        if parts[0]:  # Non-empty module name
                            ...
                            potential_init_path = str(
                                potential_init.relative_to(self.repo_path)
                            )
                        ...
                        if parts:
                            ...
                            for path in self.file_map:
                                if path.endswith(f"{parts[0]}.py") or path.endswith(
...
class ProjectBuilder:
    ...
    async def detect_project_type(self) -> Dict[str, Any]:
        ...
        has_requirements_txt = any(
            path.endswith("requirements.txt") for path in file_map
        )
        ...
        if has_package_json:
            try:
                package_json_path = next(
                    path for path in file_map if path.endswith("package.json")
                )
                async with aiofiles.open(
                    self.project_structure.repo_path / package_json_path, "r"
                ) as f:
        ...
        if has_requirements_txt:
            try:
                req_path = next(
                    path for path in file_map if path.endswith("requirements.txt")
                )
                async with aiofiles.open(
                    self.project_structure.repo_path / req_path, "r"
                ) as f:
    ...
    async def configure_build(self, project_type: str = None):
        ...
        if project_type == "react-app":
            build_config["commands"] = ["npm install", "npm run build"]
            ...
            build_config["commands"] = ["npm install", "ng build --prod"]
            ...
            build_config["commands"] = ["pip install -r requirements.txt"]
            build_config["artifacts"] = ["__pycache__/"]

        elif (
            project_type == "django-app"
            or project_type == "flask-app"
            or project_type == "fastapi-app"
        ):
            ...
            build_config["commands"] = ["cargo build --release"]
            build_config["artifacts"] = ["target/release/"]

        elif project_type == "go-project":
            build_config["commands"] = ["go build -o app"]
    ...
    async def execute_build(self) -> Dict[str, Any]:
        ...
        for cmd in self.build_config["commands"]:
            ...
            try:
                ...
                if process.returncode == 0:
                    ...
                    log_message(
                        f"Command failed: {cmd} (exit code: {process.returncode})"
                    )
        ...
        log_message(
            f"Build {build_id} completed with status: {self.current_build['status']}"
        )
...
class ProjectTester:
    ...
    async def discover_tests(self) -> Dict[str, List[str]]:
        ...
        log_message(
            f"Discovered {sum(len(tests) for tests in tests_by_type.values())} test files"
        )
        return tests_by_type

    async def map_tests_to_code(
        ...
        for file_type, tests in test_files.items():
            for test_file in tests:
                code_file = await self._find_corresponding_code_file(
                    test_file, file_type
                )
                if code_file:
    ...
    async def _find_corresponding_code_file(
        self, test_file: str, file_type: str
    ) -> Optional[str]:
        ...
        if file_type == "python":
            if test_name.startswith("test_"):
                ...
                potential_paths = [
                    ...
                    str(
                        (test_dir / "..")
                        .resolve()
                        .relative_to(self.project_structure.repo_path)
                        / code_name
                    ),
                ]
            ...
            if ".test." in test_name or ".spec." in test_name:
                ...
                potential_paths = [str(test_dir / code_name)]
            else:
                return None
        ...
        for path, info in file_map.items():
            if info.get("type") == file_type and not self._is_test_file(
                path, file_type
            ):
    ...
    async def run_tests(self, specific_tests: List[str] = None) -> Dict[str, Any]:
        ...
        for test_file in tests_to_run:
            file_type = self.project_structure.file_map.get(test_file, {}).get(
                "type", "unknown"
            )
            ...
            try:
                ...
                if not validation["valid"]:
                    ...
                    if cmd:
                        ...
                        test_result["error"] = (
                            "No test command available for this file type"
                        )
        ...
        success_rate = (
            test_run["summary"]["passed"] / test_run["summary"]["total"] * 100
            if test_run["summary"]["total"] > 0
            else 0
        )
        log_message(
            f"Test run {build_id} completed: {test_run['summary']['passed']}/{test_run['summary']['total']} passed ({success_rate:.1f}%)"
        )
    ...
    def get_test_results(self, test_file: str = None) -> Dict[str, Any]:
        ...
        if test_file:
            return self.test_results.get(
                test_file, {"status": "unknown", "message": "No test results available"}
            )
    ...
    async def analyze_test_coverage(self) -> Dict[str, Any]:
        ...
        for file_path, info in self.project_structure.file_map.items():
            ...
            if (
                ...
                and not self._is_test_file(file_path, file_type)
            ):
        ...
        coverage_percentage = (
            len(covered_files) / len(code_files) * 100 if code_files else 0
        )
...
class ProjectManager:
    ...
    async def build_project(self):
        ...
        if build_result["status"] == "success":
            ...
            self._add_event(
                f"Build failed: {build_result.get('failure_reason', 'Unknown error')}"
            )
    ...
    async def test_project(self, specific_tests: List[str] = None):
        ...
        if test_result["status"] == "passed":
            ...
            self._add_event(
                f"Tests failed: {test_result['summary']['failed']} failures, {test_result['summary']['error']} errors"
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "latest_test_run": (
                self.tester.get_latest_test_run() if self.tester else None
            ),
