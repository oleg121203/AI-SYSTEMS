...
mkdir -p "${ROOT_DIR}/web/frontend"
cat >"${ROOT_DIR}/web/frontend/.env" <<EOF
...
mkdir -p "${ROOT_DIR}/web/frontend/src"
cat >"${ROOT_DIR}/web/frontend/src/config.js" <<EOF
...
if [ -d "${ROOT_DIR}/web/frontend" ]; then
  cd "${ROOT_DIR}/web/frontend" || {
    echo -e "${RED}Could not change to Web Frontend directory.${NC}"
    exit 1
  }
  npm install
  ...
  echo -e "${RED}Web Frontend directory not found at ${ROOT_DIR}/web/frontend${NC}"
  # Continue without starting frontend
  WEB_FRONTEND_PID=0
fi
