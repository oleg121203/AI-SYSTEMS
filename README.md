...
5. **Тестування (GitHub Actions -> AI3 -> MCP API -> AI1):**
   * Коміти, зроблені MCP API в репозиторій проекту (`AI-SYSTEMS-REPO`), **не запускають** автоматично воркфлоу в основному репозиторії (`AI-SYSTEMS`).
   * Воркфлоу `.github/workflows/python-tests.yml` в репозиторії `AI-SYSTEMS` **має бути запущений** (наприклад, вручну, за розкладом, або через подію `repository_dispatch`, ініційовану з `AI-SYSTEMS-REPO`) для тестування коду.
   * Під час виконання воркфлоу завантажує код з обох репозиторіїв (`AI-SYSTEMS` в `main/`, `AI-SYSTEMS-REPO` в `repo/`).
   * GitHub Actions запускає `pytest` та лінтери для коду в `repo/`.
