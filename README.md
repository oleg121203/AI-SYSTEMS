...
### Конфігурація провайдерів для генерації структури

У файлі `config.json` визначається спеціальний список провайдерів для генерації структури:

```json
"ai3": {
  "providers": ["ollama1", "groq", "together", "openrouter-qwen"],
...
4. **Обробка звітів та оновлення статусу (MCP API):**
   ...
   * **Автоматичне створення наступних завдань (Планується):** Після отримання звіту з кодом від `executor`, *передбачається*, що MCP API автоматично створюватиме та ставитиме у чергу нові завдання для `tester` та `documenter` для цього ж файлу. Така послідовність дозволить паралельно працювати всім AI2-воркерам, підвищуючи загальну ефективність системи.
...
5. **Тестування (MCP API -> GitHub Actions -> AI3 -> MCP API -> AI1):**
   ...
   * **Воркфлоу безпосередньо запускає `pytest` та лінтери** (flake8, black, mypy, eslint, stylelint, htmlhint тощо) для коду в `repo/` (модульні/інтеграційні тести). *Примітка: він не використовує окремий скрипт `run_tests.sh`.*
...
AI1 не лише використовує алгоритмічну логіку, але й *спирається на результати аналізу LLM* (переважно, отримані від AI3) в ключових точках прийняття рішень:

1. **Аналіз результатів тестування**:
   - AI1 використовує *рекомендацію AI3* (яка базується на аналізі LLM) та власну логіку для вирішення: прийняти код (`accept`), відправити на доопрацювання (`rework`) або вимагати ручного огляду (`manual_review`).
   - Враховується контекст: рекомендація AI3, файли з помилками, історія доопрацювань, загальна мета проекту.

2. **Пріоритезація завдань**:
   - AI1 використовує алгоритмічну логіку та *може враховувати аналіз LLM* (наприклад, складність завдання, надану AI3) для визначення пріоритетів завдань (executor, tester, documenter) в кожному циклі.
   - Враховується поточний стан проекту, залежності між завданнями, статистика помилок.

3. **Генерація тексту завдань**:
   - AI1 може використовувати LLM для формування детальних інструкцій для AI2, особливо при призначенні завдань на доопрацювання *на основі аналізу помилок від AI3*.

Цей підхід робить систему більш адаптивною, здатною приймати обґрунтовані рішення, зберігаючи при цьому структурованість процесу.
...
# ./new_restart.sh --target "Опис вашого проекту" # Або використовуйте конфігурацію в config.json
...
* **Автоматичне створення наступних завдань:** *Планується*, що система автоматично створюватиме завдання для тестувальника та документатора одразу після отримання коду, забезпечуючи безперервний процес розробки.
